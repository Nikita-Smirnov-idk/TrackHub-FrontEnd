...
class CheckValidSignInData: ObservableObject {
    @Published var user: SignInUserModel
    
    // Пример проверки: все поля должны быть заполнены и соответствовать формату
    var isFormValid: Bool {
        return  user.email.isValidEmail && !user.password.isEmpty
    }
